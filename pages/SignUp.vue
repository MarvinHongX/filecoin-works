...
const handleSignUp = async () => {
    ...
    try {
        ...
        console.error(error);
        errors.value.signUp = '회원가입에 실패했습니다. 다시 시도해주세요.';
        return;
    }
    errors.value.signUp = '회원가입에 실패했습니다. 다시 시도해주세요.';
}
...
onMounted(() => {
    agreeToken.value = route.query.agree.toString();
});
...
<template>
    <div class="aaa  surface-ground flex align-items-center justify-content-center min-h-screen min-w-screen overflow-hidden">
        <div class="flex flex-column align-items-center justify-content-center">
            <div style="min-width: 20rem; max-width: 27rem; border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                <div class="w-full surface-card py-8 px-5 sm:px-8" style="border-radius: 53px">
                    <a @click="goToSignInPage()">
                        <img :src="logoUrl" alt="logo" class="mb-5 w-6rem flex-shrink-0 cursor-pointer" />
                    </a>
                    <div class="text-900 text-3xl font-medium mb-3">회원가입</div>
                    <form @submit.prevent="handleSignUp">
                        <div class="mb-3">
                            <InputGroup>
                                ...
                                <InputText id="userId1" v-model="userId" type="text" placeholder="아이디" :invalid="invalidUserId" class="w-full" style="padding: 1rem" />
                            </InputGroup>
                            <InputGroup>
                                <InputGroupAddon>
                                    <i class="pi pi-lock"></i>
                                </InputGroupAddon>
                                <Password id="password1" v-model="password" placeholder="비밀번호" :invalid="invalidPassword" :toggleMask="true" class="w-full" inputClass="w-full" :inputStyle="{ padding: '1rem' }"></Password>
                            </InputGroup>
                            <InputGroup class="mb-3">
                                <InputGroupAddon>
                                    <i class="pi pi-lock"></i>
                                </InputGroupAddon>
                                <Password id="confirmPassword1" v-model="confirmPassword" placeholder="비밀번호 확인" :invalid="invalidPassword" :toggleMask="true" class="w-full" inputClass="w-full" :inputStyle="{ padding: '1rem' }"></Password>
                            </InputGroup>
                            <div>
                        ...
                        <div class="mb-8">
                            <InputGroup>
                                ...
                                <InputText id="name1" v-model="name" type="text" placeholder="이름" :invalid="invalidName" class="w-full" style="padding: 1rem" />
                            </InputGroup>
                            <InputGroup>
                                <InputGroupAddon>
                            ...
                            <InputGroup>
                                ...
                                <InputText id="phoneNumber1" v-model="phoneNumber" type="text" placeholder="휴대전화번호" :invalid="invalidPhone" class="w-full" style="padding: 1rem" />
                            </InputGroup>
                            <div>
                        ...
                        <Button type="submit" label="다음" class="w-full p-3 text-xl mb-5"></Button>
                        <div>
